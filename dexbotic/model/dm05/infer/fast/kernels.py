...
# Fixed Gemma3-4B prefix decoder shape supported by this implementation.
...
_DEFAULT_LAUNCH_CONFIG = KernelLaunchConfig()
...
def _suffix_launch_config(buffers: DM05FusedKernelBuffers) -> KernelLaunchConfig:
...
def _row_grid(rows: int) -> tuple[int]:
    return (min(int(rows), _DEFAULT_LAUNCH_CONFIG.max_row_programs),)


def _matmul_grid(rows: int, cols: int) -> tuple[int]:
    block_m, block_n, _, _ = _DEFAULT_LAUNCH_CONFIG.matmul
...
def launch_prefix_image_merge(
    ...
    *,
    ...
    block_scan, block_h, num_warps = _DEFAULT_LAUNCH_CONFIG.prefix_image
...
def project_qkv_and_launch_qknorm_rope_split(
    ...
    buffers: DM05FusedKernelBuffers | DM05PrefixDecoderBuffers,
...
def launch_qknorm_rope_split(
    ...
    *,
    ...
    """Apply the fused Q/K norm and RoPE kernel to pre-projected QKV."""

    shape = buffers.shape
    rows = buffers.rows
    qkv = qkv.contiguous().view(rows, shape.total_qkv)
    if qkv.data_ptr() != buffers.qkv.data_ptr():
        buffers.qkv.copy_(qkv)
    _qkv_qknorm_rope_split_kernel[
        (
            min(
                rows * (shape.q_heads + 2 * shape.kv_heads),
                _DEFAULT_LAUNCH_CONFIG.max_row_programs,
            ),
        )
    ](
        buffers.qkv,
        ...
        num_warps=_DEFAULT_LAUNCH_CONFIG.qkv_warps,
    )
...
def launch_input_adarmsnorm(
    ...
    buffers: DM05FusedKernelBuffers,
    *,
    eps: float,
) -> None:
    shape = buffers.shape
    block_h, num_warps = _DEFAULT_LAUNCH_CONFIG.norm
...
def launch_suffix_attn_postnorm_residual_prenorm(
    ...
    *,
    ...
    rows = buffers.rows
    hidden = shape.hidden_size
    norm_block_h, norm_warps = _DEFAULT_LAUNCH_CONFIG.norm
...
def launch_geglu_gate_up(
    ...
    rows = buffers.rows
    hidden = shape.hidden_size
    mm_block_m, mm_block_n, mm_warps, mm_stages = _DEFAULT_LAUNCH_CONFIG.matmul
    _geglu_gate_up_kernel[_matmul_grid(rows, shape.intermediate_size)](
        ...
        BLOCK_M=mm_block_m,
        BLOCK_N=mm_block_n,
        BLOCK_K=_DEFAULT_LAUNCH_CONFIG.geglu_block_k,
        num_warps=mm_warps,
...
def launch_prefix_geglu_from_gate_up(
    ...
    block_m, block_n, mm_warps, mm_stages = _DEFAULT_LAUNCH_CONFIG.matmul
