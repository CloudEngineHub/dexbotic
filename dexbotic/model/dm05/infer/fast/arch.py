...
and dispatches the fused Triton kernel path on CUDA.
"""
...
from dexbotic.model.dm05.dm05_arch import DM05ActionExpert, DM05ForConditionalGeneration
...
try:
    import dexbotic.model.dm05.infer.fast.kernels as triton_kernels
except ImportError as exc:
    triton_kernels = None
    _TRITON_KERNELS_IMPORT_ERROR = exc
else:
    _TRITON_KERNELS_IMPORT_ERROR = None
...
def _prefix_decoder_shape(layer) -> "triton_kernels.DM05TransformerShape":
    attn = layer.self_attn
    head_dim = int(attn.head_dim)
    return triton_kernels.DM05TransformerShape(
...
def _supported_prefix_decoder_shape() -> "triton_kernels.DM05TransformerShape":
    return triton_kernels.DM05TransformerShape(
        hidden_size=triton_kernels.SUPPORTED_PREFIX_HIDDEN_SIZE,
        intermediate_size=triton_kernels.SUPPORTED_PREFIX_INTERMEDIATE_SIZE,
        head_dim=triton_kernels.SUPPORTED_PREFIX_HEAD_DIM,
        q_heads=triton_kernels.SUPPORTED_PREFIX_Q_HEADS,
        kv_heads=triton_kernels.SUPPORTED_PREFIX_KV_HEADS,
    )
...
class DM05FastActionExpert(DM05ActionExpert):
    """Inference-only action expert using the fused Triton kernel path."""
    ...
    def setup_fused_triton_suffix(self) -> None:
        if triton_kernels is None:
            raise ImportError(
                "The fused Triton kernel path requires triton to be installed."
            ) from _TRITON_KERNELS_IMPORT_ERROR
    ...
    def make_fast_prefix_buffers(self, prefix_seq_len: int):
        ...
        return triton_kernels.make_prefix_decoder_buffers(
            batch_size=1,
    ...
    def make_fast_suffix_buffers(
        *,
        ...
        return triton_kernels.make_suffix_buffers(
    ...
    def _suffix_forward_layers_fused_triton(
        self,
        ...
        if buffers is None:
            raise RuntimeError(
                "Fast suffix buffers must be initialized before suffix forward."
            )
        ...
        for layer_idx, (layer_cache_keys, layer_cache_values) in enumerate(
            ...
            hidden_states = self._compute_suffix_layer_fused_triton(
                layer_idx,
                hidden_states,
                prefix_visible_mask,
                layer_cache_keys,
    ...
    def _compute_suffix_layer_fused_triton(
        self,
        layer_idx: int,
        suffix_embeds: torch.Tensor,
        prefix_visible_mask: torch.Tensor,
        ...
        *,
        ...
        triton_kernels.launch_input_adarmsnorm(
            suffix_embeds,
            input_modulation,
            buffers,
            eps=float(ae_layer.input_layernorm.eps),
        )
        triton_kernels.project_qkv_and_launch_qknorm_rope_split(
            buffers.mlp_input,
        ...
        triton_kernels.launch_suffix_attention(
            buffers.query,
            buffers.key,
            buffers.value,
            cache_keys,
        ...
        triton_kernels.launch_suffix_attn_postnorm_residual_prenorm(
            attn_embeds,
            suffix_embeds,
            attn_gate,
        ...
        triton_kernels.launch_geglu_gate_up(
            buffers.mlp_input,
            gate_weight,
            up_weight,
            buffers.mlp_hidden,
        ...
        triton_kernels.launch_mlp_down(
            buffers.mlp_hidden,
            down_weight,
            buffers.mlp_out,
            buffers,
        ...
        return triton_kernels.launch_suffix_post_ffn_norm_residual(
            mlp_gate,
...
class DM05FastForCausalLM:
    """Model-side prefill/decode schedule used by the DM05 fast backend."""
    ...
    def __init__(
        ...
        *,
        ...
        if prefix_qkv_mode not in {"packed", "separate"}:
        ...
        # Maximum multimodal prefix length supported by the DM05 fast backend.
        ...
        self.prefix_qkv_mode = prefix_qkv_mode
        ...
        self.action_expert.setup_fused_triton_suffix()
    ...
    def setup_fast_prefix_decoder(self) -> None:
        ...
        self.action_expert.setup_fused_triton_prefix(
            layers,
        )
    ...
    def _validate_prefix_inputs(
        ...
        *,
        ...
        if enforce_max_length and input_len > self.prefix_len:
            raise ValueError(
    ...
    def _zero_history_pad_embeds(
        ...
        """Zero ``<unused1>`` positions reserved for padded history tokens.
    ...
    def _inject_history_into_embeds(
        ...
        *,
        ...
        """Scatter pooled history features into the ``<unused0>`` positions."""
    ...
    def prefill_fast_from_image_features(
        ...
        *,
        ...
        triton_kernels.launch_prefix_image_merge(
            input_ids,
            image_features,
            inputs_embeds,
        ...
        for layer_idx, decoder_layer in enumerate(language_model.layers[:layer_count]):
            ...
            if self.prefix_qkv_mode == "packed":
                triton_kernels.project_qkv_and_launch_qknorm_rope_split(
                    hidden_states,
                ...
                triton_kernels.launch_qknorm_rope_split(
                    projected_qkv,
                    prefix_buffers,
                    attention.q_norm.weight,
                    attention.k_norm.weight,
            ...
            triton_kernels.launch_prefix_attn_postnorm_residual_prenorm(
                hidden_states,
                residual,
                prefix_buffers,
            ...
            hidden_states = triton_kernels.launch_prefix_post_ffn_norm_residual(
                prefix_buffers,
    ...
    def prepare_fast_suffix_context(
        ...
        *,
        ...
        if initial_noise.device.type != "cuda":
            raise RuntimeError("The fused Triton kernel path requires CUDA tensors.")
        ...
        if suffix_buffers is None:
            max_kv_len = int(prefix_cache_keys[0].shape[2]) + suffix_len
    ...
    def decode_fast_prepared(
        ...
        *,
        ...
        for step_idx in range(int(diffusion_steps)):
            ...
            suffix_out = self.action_expert._suffix_forward_layers_fused_triton(
